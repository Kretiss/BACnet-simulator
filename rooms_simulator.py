...
from bacpypes.basetypes import DeviceObjectPropertyReference, DateRange, DailySchedule, TimeValue
...
parser.add_argument("-st", "--sleepTime", default=0.4, type=int, help="Main loop sleep time (simulation speed)")
...
# bacnet = lite(ip=args["address"], port=args["port"] + 1)
    ...
    properties={
        ...
        "weeklySchedule": ArrayOf(DailySchedule)(
            [
                DailySchedule(
                    daySchedule=[
                        TimeValue(time=(8, 0, 0, 0), value=Real(23)),
...
#
# _new_objects = LocalScheduleObject(
...
# deviceAddress += ":"
# deviceAddress += str(args["port"])
# bacnet.write_weeklySchedule(deviceAddress, 10001, schedule)
# bacnet = None
...
inner_wall_sigma = 0.8  # objemová hmotnost 1700kg/m3, měrná tepelná kapacita 840 J/kg*K, řádek 60 - 2 web
...
radiator_heat_transfer_const = 229 * 0.25  # Hliníkový radiátor
...
while True:
    ...
    # r1_radiator_heat_flow = radiator_heat_transfer_const * (Real(45) - r1_temperature) * r1_radiator_state
    r1_radiator_heat_flow = radiator_power * r1_radiator_state
    r1_radiator_temp_diff = 1 / room_capacity * r1_radiator_heat_flow

    # r2_radiator_heat_flow = radiator_heat_transfer_const * (Real(45) - r2_temperature) * r2_radiator_state
